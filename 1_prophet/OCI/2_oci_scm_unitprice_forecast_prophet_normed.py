# -*- coding: utf-8 -*-
"""2. OCI_scm_unitprice_forecast_prophet_normed.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1YCtIGNdsoZdRY9jw00PbMN-H6m1iYC-_
"""
...
df_mean = df["y"].mean()
df_std = df["y"].std()
df_norm = (df["y"] - df_mean) / df_std
df["y_norm"] = df_norm
df.head()

print(f"y_max : {df['y'].max()}, y_min : {df['y'].min()}")
print(f"y_norm_max : {df['y_norm'].max()}, y_norm_min : {df['y'].min()}")

sns.distplot(df["y_norm"])
...
pd.plotting.register_matplotlib_converters()
f, ax = plt.subplots(figsize=(14,5))
X_tr.plot(kind='line', x='ds', y='y_norm', color='blue', label='Train', ax=ax)
X_tst.plot(kind='line', x='ds', y='y_norm', color='red', label='Test', ax=ax)
plt.title('normed Sales Amount Traning and Test data')
plt.show()
...
X_tr = X_tr.rename(columns={"y_norm": "y"})

X_tst.drop(columns = ["y"], axis=1, inplace=True)
X_tst = X_tst.rename(columns={"y_norm": "y"})
...
df.to_csv("/gdrive/My Drive/colab/Sale_data/dataset/preprocessed_data.csv")

forecast.to_csv("/gdrive/My Drive/colab/Sale_data/dataset/forecast_train.csv")

X_tst_final.to_csv("/gdrive/My Drive/colab/Sale_data/dataset/forecast_test_result.csv")
